def call(String version, boolean noSpark = false, String docs_commitid = 'latest', String customerPkg = 'NORMAL', boolean excludeDocs = false) {
    ...
    if (noSpark) {
        ...
        sh "export ${customerPkg}=1 && export release_version=${version} && sh build/script_newten/release.sh -noTimestamp -noSpark -Dmaven.wagon.http.ssl.insecure=true"
    } else {
        if (!excludeDocs) {
            insertDocs(version, docs_commitid)
        }

        sh "export ${customerPkg}=1 && export release_version=${version} && sh build/script_newten/release.sh -noTimestamp -Dmaven.wagon.http.ssl.insecure=true"
    }
}
