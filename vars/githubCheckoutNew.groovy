def call(String project, String branch = null, String schema = 'git', String remoteHost = '10.1.2.192', 
         refspec = '+refs/heads/*:refs/remotes/origin/*', certificate=null, repo=null) {
    println "githubCheckoutNew -> params: project: ${project}, branch: ${branch}, schema: ${schema}, remoteHost: ${remoteHost}, refspec: ${refspec}, certificate: ${certificate}, repo: ${repo}"
    timestamps {
        checkout([
            $class: 'GitSCM', 
            branches: [[name: "${branch ?: sha1}"]], 
            extensions: [[$class: 'CloneOption', depth: 1, honorRefspec: true, noTags: true, reference: '', shallow: true, timeout: 15]], 
            userRemoteConfigs: [[credentialsId: "${certificate ?: 'kyligence-git'}", name: 'origin', 
                                refspec: "${getRefspec(branch,sha1)}", 
                                url: "${schema ?: 'git'}://${remoteHost ?: '10.1.2.192'}/${repo ?: 'Kyligence'}/${project}.git"]]])
        checkLicensePatch()
    }
}

def getRefspec(String branch, String sha1) {
    def refs = ''
    if(branch){
        refs = '+refs/heads/'+branch+':refs/remotes/origin/'+branch
    } else if(sha1) {
        refs = '+refs/pull/'+matchPullNumber(sha1)+'/*:refs/remotes/origin/pr/'+matchPullNumber(sha1)+'/*'
    } else {
        refs = '+refs/heads/*:refs/remotes/origin/*'
    }
}

def checkLicensePatch() {
    sh script: '''
    if [ -f license.patch ]; then
        git apply license.patch
        git checkout -- .
    fi
    '''
}

def matchPullNumber(String sha1) {
    def m = sha1 =~ /^origin\/pr\/(\d+)\/merge$/
    if (!m) {
        throw new RuntimeException("invalid sha1")
    }
    return m.group(1)
}