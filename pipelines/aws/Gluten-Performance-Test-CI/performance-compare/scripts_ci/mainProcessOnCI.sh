...
echo "$(date '+%F %T'): shutdown cloud vms and hdfs cluster at start!"
...
if test -s /tmp/service;then
  export service=($(cat /tmp/service)) #service type which user passed by jenkins parameter,override default 'GlutenWithCHStandard'
fi
...
if [ ${run_mode} = "aws" ];then
  ...
  if [ ${need_prepare_emr} -eq 1 ];then
    ...
    sourceType=$(cat /tmp/source_type)
    if [[ ${sourceType} == Remote* ]];then
...
echo ${driver_host} > /tmp/driver_ip
...
do
    ...
    if [ \$? -ne 0 ];then
	  ...
	  #prepare for ch compile env
	  bash ./setupClickhouseCompileEnv.sh

	  #prepare tools for performance comparison
	  sudo apt install -y numactl parallel


	  ps -ef|grep http.server|grep -v grep
	  if [ \$? -ne 0 ];then
      cd ${report_home}
      nohup python3 -m http.server 5678 > webserver.log 2>&1 &
      cd -
    fi
...
ps -ef|grep "statistic.py"|grep -v grep|awk '{print $2}'|xargs kill -9
#upload all results to conbench,tbd
echo "$(date '+%F %T'): upload result to conbench"
sudo apt install -y ksh
ksh -x ${script_home}/uploadResultToConbench.sh
