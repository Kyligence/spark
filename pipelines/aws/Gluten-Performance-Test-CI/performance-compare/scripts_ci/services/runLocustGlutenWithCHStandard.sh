...
sourceType=$(cat /tmp/source_type) # RemoteNullableParquet LocalNullableParquet LocalNullableMergetree LocalNotNullMergertree
if [ "${sourceType}" == "RemoteNullableParquet" ];then
  echo "$(date '+%F %T'): GlutenWithCHStandard create RemoteNullableParquet table and make query test"
  python3 ${locust_home}/statistic.py --iterations ${iteration} --sql-path ${sqls_home} --dialect gluten --engine gluten --output-dir ${result_dir} \
    -p 10000 --host ${private_driver_host} --user root --database tpch100_remote_nullable_parquet --external-path hdfs://${private_namenode_ip}:8020/tmp/tpch-data-sf100 \
    --data-format parquet --drop-table-before-create True --column-nullable True --batch tpch100 --compare-result true
elif [ "${sourceType}" == "LocalNullableParquet" ];then
  echo "$(date '+%F %T'): GlutenWithCHStandard create LocalNullableParquet table and make query test"
  python3 ${locust_home}/statistic.py --iterations ${iteration} --sql-path ${sqls_home} --dialect gluten --engine gluten --output-dir ${result_dir} \
    -p 10000 --host ${private_driver_host} --user root --database tpch100_local_nullable_parquet --external-path file:///${data_home}/tpch-data-sf100 \
    --data-format parquet --drop-table-before-create True --column-nullable True --batch tpch100 --compare-result true
elif [ "${sourceType}" == "LocalNullableMergetree" ];then
  echo "$(date '+%F %T'): GlutenWithCHStandard create LocalNullableMergetree table and make query test"
  python3 ${locust_home}/statistic.py --iterations ${iteration} --sql-path ${sqls_home} --dialect gluten --engine gluten --output-dir ${result_dir} \
    -p 10000 --host ${private_driver_host} --user root --database tpch100_local_nullable_mergetree --external-path ${data_home}/tpch100_ch_data/nullable \
    --data-format mergetree --drop-table-before-create False --column-nullable True --batch tpch100 --compare-result true
elif [ "${sourceType}" == "LocalNotNullMergertree" ];then
  echo "$(date '+%F %T'): GlutenWithCHStandard create LocalNotNullMergertree table and make query test"
  python3 ${locust_home}/statistic.py --iterations ${iteration} --sql-path ${sqls_home} --dialect gluten --engine gluten --output-dir ${result_dir} \
    -p 10000 --host ${private_driver_host} --user root --database tpch100_local_notnull_mergetree --external-path ${data_home}/tpch100_ch_data/notnull \
    --data-format mergetree --drop-table-before-create False --column-nullable False --batch tpch100 --compare-result true
fi
...
#    ansible --key-file ${key_file} workers  -m copy -a "src=${data_home}/tpch100_ch_data/notnull/${table}/_delta_log dest=${data_home}/tpch100_ch_data/notnull/${table}/"
#  done
#  # sleep 10000
