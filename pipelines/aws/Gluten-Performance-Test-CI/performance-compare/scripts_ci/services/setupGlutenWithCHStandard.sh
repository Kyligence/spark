...
sleep 10
./bin/beeline -u jdbc:hive2://${spark_master_ip}:10000/ -n root -e "create database if not exists tpch100_remote_nullable_parquet;create database if not exists tpch100_local_nullable_parquet;create database if not exists tpch100_local_nullable_mergetree;create database if not exists tpch100_local_notnull_mergetree;"
if [ $? -ne 0 ];then
	sleep 15
	./bin/beeline -u jdbc:hive2://${spark_master_ip}:10000/ -n root -e "create database if not exists tpch100_remote_nullable_parquet;create database if not exists tpch100_local_nullable_parquet;create database if not exists tpch100_local_nullable_mergetree;create database if not exists tpch100_local_notnull_mergetree;"
	if [ $? -ne 0 ];then
