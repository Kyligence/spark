...
class JDBCV2Suite extends QueryTest with SharedSparkSession with ExplainSuiteHelper {
  ...
  override def beforeAll(): Unit = {
    ...
    withConnection { conn =>
      ...
      conn.prepareStatement(
        "CREATE TABLE \"test\".\"dept\" (\"dept_id\" INTEGER NOT NULL, \"dept.id\" INTEGER)")
        .executeUpdate()
  ...
  test("column name with composite field") {
    checkAnswer(sql("SELECT `dept_id`, `dept.id` FROM h2.test.dept"), Seq(Row(1, 1), Row(2, 1)))
    ...
    val df2 = sql("SELECT `dept.id`, COUNT(`dept_id`) FROM h2.test.dept GROUP BY `dept.id`")
    checkPushedInfo(df2,
      "PushedGroupByExpressions: [`dept.id`]", "PushedAggregates: [COUNT(`dept_id`)]")
    checkAnswer(df2, Seq(Row(1, 2)))

    val df3 = sql("SELECT `dept_id`, COUNT(`dept.id`) FROM h2.test.dept GROUP BY `dept id`")
    checkPushedInfo(df3,
      "PushedGroupByExpressions: [`dept_id`]", "PushedAggregates: [COUNT(`dept.id`)]")
