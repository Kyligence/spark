...
class JDBCV2Suite extends QueryTest with SharedSparkSession with ExplainSuiteHelper {
  ...
  test("column name with composite field") {
    ...
    checkPushedInfo(df2,
      "PushedGroupByExpressions: [dept_id1]",
      "PushedAggregates: [COUNT(dept_id)]")
    checkAnswer(df2, Seq(Row(1, 2)))

    val df3 = sql("SELECT `dept_id`, COUNT(`dept_id1`) FROM h2.test.dept GROUP BY `dept_id`")
    checkPushedInfo(df3,
      "PushedGroupByExpressions: [dept_id]",
      "PushedAggregates: [COUNT(dept_id1)]")
  ...
  test("scan with aggregate push-down: partial push-down AVG with overflow") {
    Seq(true, false).foreach { ansiEnabled =>
      withSQLConf((SQLConf.ANSI_ENABLED.key, ansiEnabled.toString)) {
        ...
        checkAggregateRemoved(df, false)
        checkPushedInfo(df, "PushedAggregates: [SUM(PRICE), COUNT(PRICE)]")
        if (ansiEnabled) {
          val e = intercept[SparkException] {
            df.collect()
          }
