...
import org.apache.spark.sql.catalyst.expressions.{ExpressionEvalHelper, Literal, TimestampAdd}
...
class DateTimeUtilsSuite extends SparkFunSuite with Matchers with SQLHelper
  ...
  test("SPARK-42635: timestampadd unit conversion overflow") {
    withSQLConf(SQLConf.SESSION_LOCAL_TIMEZONE.key -> "UTC") {
      checkExceptionInExpression[SparkArithmeticException](TimestampAdd(Literal("DAY"),
        Literal(106751992),
        Literal(0L, TimestampType)),
        "Datetime operation overflow")
      checkExceptionInExpression[SparkArithmeticException](TimestampAdd(Literal("QUARTER"),
        Literal(1431655764),
        Literal(0L, TimestampType)),
        "Datetime operation overflow")
    }
