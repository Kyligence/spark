    ...
    void process(Csv data) {
        ...
        data.getRows().forEach(row -> {
            ...
            row.forEach(col -> {
                ...
                result.addRow([idCol, runDateCol, JenkinsAnalysis.Column.valueOf("moduleName", col.getName().replace(SUFFIX, "")), JenkinsAnalysis.Column.valueOf("testState", col.getValue())])
            })
        })
        ...
        s3client.putObject(config.getS3StoreBucket(), "${String.format(DATA_KEY_PREFIX, compatiblePlatform(config.getPlatform()), "KE4", "Newten_CI_On_GCP")}/module_test_states/data.csv", dataStream, meta)
    }
}
