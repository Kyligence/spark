...
@Slf4j
class LightningBasicPostAnalyzer extends AbstractPostAnalyzer {

    LightningBasicPostAnalyzer(JenkinsConfig config, AmazonS3 s3client) {
    ...
    void process(JenkinsTransform.Csv data) {
        ...
        s3client.putObject(config.getS3StoreBucket(), "${String.format(DATA_KEY_PREFIX, config.getPlatform(), "KC", "Lightning-PR-CHECK")}/basic/data.csv", dataStream, meta)
    }
}
